      ...
      <el-col :span="8">
        <el-card class="input-card">
          ...
          <el-form :model="planForm" :rules="rules" ref="planForm" label-width="100px">
            ...
            <el-form-item label="途经点" v-if="planForm.waypoints.length > 0">
              <div class="waypoints">
              ...
              <el-button @click="addWaypoint" type="primary" size="mini" icon="el-icon-plus">添加途经点</el-button>
            </el-form-item>
  ...
  methods: {
    ...
    handlePlan() {
      this.$refs['planForm'].validate((valid) => {
        if (valid) {
          // 调用后端API进行线路规划
          this.request.post("/route/plan", this.planForm).then(res => {
            if (res.code === '200') {
    ...
    confirmSelectRoute() {
      if (this.selectedRoute) {
        // 保存选中的线路到后端
        this.request.post("/route", this.selectedRoute).then(res => {
          if (res.code === '200') {
    ...
    handleSavePlan() {
      ...
      // 保存规划方案到后端
      this.request.post("/route/savePlan", {
