...
export default {
  ...
  methods: {
    ...
    handleAnalyze() {
      // 调用后端API进行分析
      this.request.post("/route/analysis", this.filterForm).then(res => {
        if (res.code === '200') {
          this.updateAnalysisData(res.data);
    ...
    updateAnalysisData(data) {
      ...
      if (data.popularRoutes) {
        this.popularRoutes = data.popularRoutes;
      }
    ...
    handleExport() {
      this.$confirm('确认导出线路数据吗?', '提示', {
        ...
        if (this.filterForm.dateRange && this.filterForm.dateRange.length > 0) {
          params.append('dateRange', this.filterForm.dateRange.join(','));
        }
