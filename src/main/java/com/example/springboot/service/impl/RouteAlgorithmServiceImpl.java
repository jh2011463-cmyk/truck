...
public class RouteAlgorithmServiceImpl implements IRouteAlgorithmService {
    ...
    public Map<String, Object> analyzeRoutes(Map<String, Object> params) {
        ...
        try {
            ...
            stats.put("totalRoutes", filteredRoutes.size());
            ...
            double avgRisk = filteredRoutes.stream()
                .mapToDouble(r -> assessRouteRisk(r))
                .average()
                .orElse(0.0);
            ...
            // 只返回前5条热门路线
            if (popularRoutes.size() > 5) {
                popularRoutes = popularRoutes.subList(0, 5);
            }
    ...
    private List<Route> filterRoutes(List<Route> routes, Map<String, Object> params) {
        ...
        return routes.stream()
            .filter(route -> {
                ...
                // 距离范围过滤
                if (params.containsKey("distanceRange") && params.get("distanceRange") instanceof List) {
                    List<Integer> distanceRange = (List<Integer>) params.get("distanceRange");
                    if (distanceRange.size() >= 2 && route.getDistance() != null) {
                        double minDistance = distanceRange.get(0);
                        double maxDistance = distanceRange.get(1);
                        if (route.getDistance() < minDistance || route.getDistance() > maxDistance) {
                            return false;
                        }
                    }
                }
